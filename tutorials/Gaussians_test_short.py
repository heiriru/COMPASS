...
def plot_confusion_violin_panels(comparison_stats_by_true_model, model_names, path):
    def collect_rows(run_stats):
        rows = []
        for compared_model in model_names:
            obs_probs = torch.as_tensor(run_stats[compared_model]["obs_probs"], dtype=torch.float)
            for probability in obs_probs.detach().cpu().numpy():
                rows.append(
                    {
    ...
    def draw_panel_violins(rows_by_true_model, filename, title):
        fig, axes = plt.subplots(
            1,
            len(model_names),
        ...
        if len(model_names) == 1:
            axes = [axes]
        palette = sns.color_palette("dark", n_colors=len(model_names))
        for ax, true_model_name in zip(axes, model_names):
            rows = rows_by_true_model.get(true_model_name, [])
            if not rows:
            ...
            ax.set_title(f"True model: {true_model_name}", fontsize=14)
            ax.set_xlabel("")
            ax.set_ylim(0, 1)
    ...
    for true_model_name in model_names:
        run_stats = comparison_stats_by_true_model.get(true_model_name, [])
        if not run_stats:
            continue
        first_run_rows[true_model_name] = collect_rows(run_stats[0])
        all_rows = []
        for stats in run_stats:
            all_rows.extend(collect_rows(stats))
        all_observation_rows[true_model_name] = all_rows

    if first_run_rows:
        draw_panel_violins(
            first_run_rows,
            "model_probs_violin_by_true_model_first_run.png",
            "Per-observation model probabilities by true model",
        )
    if all_observation_rows:
        draw_panel_violins(
            all_observation_rows,
            "model_probs_violin_by_true_model_all_confusion_observations.png",
            "Per-observation model probabilities across all confusion runs",
        )
...
def main():
    ...
    for true_model_name in progress(model_names, "[compare] true models", leave=True):
        ...
        for run_index, (theta_raw, observations_raw) in enumerate(
            ...
            if comparison_stats is None:
                comparison_stats = deepcopy(mtf.stats)
    ...
    run_baseline_tests_and_plots(
        ...
        comparison_stats_by_true_model=comparison_stats_by_true_model,
    )
