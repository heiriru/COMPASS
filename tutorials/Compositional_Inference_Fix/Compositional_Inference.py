...
def configure_cpu_usage_limit(max_threads: int = CPU_THREAD_LIMIT) -> tuple[int, tuple[int, ...]]:
    """Restrict this script and inherited children to at most 3 host CPU threads."""
...
SAMPLER_VARIANTS = (
    ...
    SamplerVariant("dpm_gauss_full", "DPM-2 + full Gaussian", "dpm", "full_gaussian", 5),
...
GAUSS_GLOBAL_LOCAL_MOMENT_VARIANT = SamplerVariant(
    "dpm_gauss_global_local_moment",
    "DPM-2 + global/local Gaussian moments",
    "dpm", "Gauss_global_local", 0,
)
SCALING_MOMENT_FIGURE = (
    "samplers_vs_observation_count_gauss_global_local_moment.png"
)
...
OBSERVATION_SCALING_VARIANTS = (
    ...
    PFODE_GAUSS_VARIANT,
    *SAMPLER_VARIANTS[2:],
    GAUSS_GLOBAL_LOCAL_MOMENT_VARIANT,
)
...
def known_shared_covariance_full(n: int = 1) -> torch.Tensor:
    return torch.diag_embed(1.0 / known_shared_precision(n))


def known_shared_mean(x: torch.Tensor) -> torch.Tensor:
    precision = 1.0 / S0**2 + 1.0 / SX**2
    return (MU0 / S0**2 + x / SX**2) / precision
...
def sample_shared(
    ...
    samples = model.sample(
        ...
        posterior_precision=(
            known_shared_precision(x.shape[0])
            if not automatic_precision and variant.correction == "gauss"
            else None
        ),
        posterior_covariance=(
            known_shared_covariance_full(x.shape[0])
            if not automatic_precision and variant.correction in {
                "full_gaussian", "Gauss_global_local"
            }
            else None
        ),
        posterior_mean=(
            known_shared_mean(x)
            if variant.key == GAUSS_GLOBAL_LOCAL_MOMENT_VARIANT.key
            else None
        ),
        ...
        order=2,
        method="dpm" if variant.method == "heun" else variant.method,
...
def evaluate_sampler_grid(
    ...
    for repeat in range(repeats):
        ...
        for n in n_values:
            ...
            truth_mean, truth_std = analytic_shared(x)
            all_seed_variants = (
                *SAMPLER_VARIANTS, PFODE_GAUSS_VARIANT,
                GAUSS_GLOBAL_LOCAL_MOMENT_VARIANT,
            )
...
def experiment_observation_scaling(
    ...
    figure_name = (
        SCALING_MOMENT_FIGURE
        if any(str(row["variant"]) == GAUSS_GLOBAL_LOCAL_MOMENT_VARIANT.key
               for row in rows)
        else "samplers_vs_observation_count.png"
    )
    save_figure(fig, out / figure_name)
